  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      if (!user || !user.accessToken) {
        return reply
          .status(401)
          .send({ error: 'User not authenticated or access token missing' });
      }

      try {
        fastify.log.info(
          `Request to /api/game/simulate-round-with-timer for artistId: ${artistId}`,
        );
        ...
        if (!roundData) {
          fastify.log.warn(
            `Could not generate game round for artist ${artistId}.`,
          );
          return reply
            .status(404)
            .send({
        ...
        fastify.log.info(
          `Round data prepared. Track to guess: ${roundData.trackToGuess.name}. Options: ${roundData.options.join(', ')}`,
        );
        fastify.log.info(
          `Starting 5-second timer simulation using Task 1 functions...`,
        );
        ...
        const simulatedTickCallback = (tick) => {
          ...
          fastify.log.info(
            `Tick: ${tick}, Time Elapsed: ${secondsElapsed}s, Time Left: ${timeLeft >= 0 ? timeLeft : 0}s`,
          );
          if (timeLeft <= 0) {
            fastify.log.info(`5 seconds up! (Simulated)`);
          }
        };
        ...
        return reply.send({
          ...
          trackToGuessName: roundData.trackToGuess.name,
          options: roundData.options,
        });
      } catch (error) {
        fastify.log.error(
          `Error in /api/game/simulate-round-with-timer: ${error.message}`,
        );
        if (
          error.message &&
          error.message.includes('Spotify API error') &&
          ...
          return reply
            ...
            .send({
              error:
                'Spotify token expired or invalid. Please re-authenticate.',
            });
        }
        return reply
          .status(500)
